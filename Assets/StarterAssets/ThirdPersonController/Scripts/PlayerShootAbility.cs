...
public class PlayerShootAbility : MonoBehaviour
{
    ...
    private void Start()
    {
        bulletsPool = GetComponent<GenericPool>();
        _input = GetComponent<StarterAssetsInputs>();
    }

    void Update()
    {
        if (_input.shoot && canShoot && EnergySystem.instance.HasEnoughEnergy())
        {
    ...
    void Shoot()
    {
        ...
        if (searchEnemy != null)
        {
            bullet.GetComponentInChildren<BulletPointer>().target = searchEnemy;
        }
